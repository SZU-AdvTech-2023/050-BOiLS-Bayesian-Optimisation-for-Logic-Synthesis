...
from build_in_seq.main import RefObj, BUILD_IN_SEQ
...
def get_ref(
    ...
    ref_obj = RefObj(design_file=design_file, imap_binary=imap_binary,
                     libary=libary,  ref_imap_seq=ref_imap_seq)
...
class FPGASession:
    ...
    def __init__(self, design_name: str, design_file: str, playground_dir: str, action_space_id: str,
                  imap_binary: str, ref_imap_seq: str, objective: str, max_iterations: int):
        """
        Args:
        ...
        self.observation_space_size = 5  # number of features
        ...
        init_obj = RefObj(design_file=self.design_file,  imap_binary=self.imap_binary,
                           ref_imap_seq='init')
        ...
        # self.baseline_impr_lut_k = ((self.init_lut_k - self.ref_lut_k) / self.init_lut_k) / BUILD_IN_SEQ[self.ref_imap_seq].seq_length()
        # self.baseline_impr_level = ((self.init_level - self.ref_level) / self.init_level) / BUILD_IN_SEQ[self.ref_imap_seq].seq_length()
    ...
    def _run(self):
        ...
        try:
            proc = check_output([self.imap_binary, '-c', abc_command])
            # get reward
    ...
    @staticmethod
    def get_design_prop(libary: str, design_file: str, imap_binary: str, lut_inputs: int,
